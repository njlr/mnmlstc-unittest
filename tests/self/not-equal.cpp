...
void v1() {
  ...
  try { self.assert_not_equal(1, 2); }
  catch (...) {
    std::clog << "unexpected exception was thrown" << std::endl;
    std::exit(EXIT_FAILURE);
  }

  try { self.assert_not_equal(1, 1); }
  catch (exception const& e) {
    if (std::string { "assert_not_equal" } != e.type()) {
  ...
  try { self.assert_not_equal(custom(), custom()); }
  catch (exception const& e) {
    if (std::string { "assert_not_equal" } != e.type()) {
      std::clog << "unexpected exception '" << e.type() << "' was thrown"
                << std::endl;
    ...
    std::clog << "unexpected exceptionw as thrown" << std::endl;
    std::exit(EXIT_FAILURE);
  }
}
