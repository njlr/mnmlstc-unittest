...
void v1 () {
  ...
  try { self.assert_greater_equal(2, 1); }
  catch (...) {
    std::clog << "unexpected exception was thrown" << std::endl;
    std::exit(EXIT_FAILURE);
  }

  try { self.assert_greater_equal(1, 2); }
  catch (exception const& e) {
    if (std::string { "assert_greater_equal" } != e.type()) {
  ...
  try { self.assert_greater_equal(custom(), custom()); }
  catch (exception const& e) {
    if (std::string { "assert_greater_equal" } != e.type()) {
      std::clog << "unexpected exception '" << e.type() << "' was thrown"
                << std::endl;
